...
**Diseño aprobado** (`docs/fase10-design.md`, ADR-024). Componente `featured` en el page-schema
(reutiliza `SectionDataResolver` + canal `resolved` + `EntryCard`): props = referencias (ULIDs
ordenados de artículos), datos resueltos en el backend. `FeaturedResolver` (whereIn + reorden en
memoria, anti-N+1; sólo publicados). Curación **drag-and-drop** en el Builder (control
